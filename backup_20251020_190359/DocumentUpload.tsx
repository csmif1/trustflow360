...
export default function DocumentUpload() {
  ...
  const processDocument = async () => {
    ...
    try {
      ...
      // Step 2: AI Processing with Gemini - USING REAL PROCESSOR
      ...
      try {
        ...
        if (result.success && result.extracted) {
          const extracted = {
            ...result.extracted,
            giftDate: result.extracted.giftDate || result.extracted.dateEstablished || new Date().toISOString().split('T')[0],
                ...
                <TabsContent value="ai" className="space-y-4 mt-6">
                  <div className="grid grid-cols-2 gap-4">
                    ...
                    <div>
                      ...
                      <p className="font-medium">{extractedData.trusteeName || "Not specified"}</p>
                    </div>
                    <div className="col-span-2">
                      <Label className="text-sm text-slate-600">Beneficiaries</Label>
                      <p className="font-medium">{extractedData.beneficiaries?.join(", ") || "None listed"}</p>
                    </div>
                    <div>
                      <Label className="text-sm text-slate-600">Initial Seed Gift</Label>
                      <p className="font-medium">${extractedData.initialSeedGift ? Number(extractedData.initialSeedGift?.replace("$", "").replace(",", "")).toLocaleString() : 0}</p>
                    </div>
                    <div>
                      <Label className="text-sm text-slate-600">Annual Gift Amount</Label>
                      <p className="font-medium">${extractedData.annualGiftAmount ? Number(extractedData.annualGiftAmount?.replace("$", "").replace(",", "")).toLocaleString() : 0}</p>
                    </div>
                    <div>
                    ...
                    <div>
                      ...
                      <p className="font-medium capitalize">{extractedData.giftType || "Cash"}</p>
                    </div>
                  </div>

                  {extractedData.giftSchedule && extractedData.giftSchedule.length > 0 && (
                    <div className="mt-4">
                      <Label className="text-sm text-slate-600 mb-2 block">Gift Schedule</Label>
                      <div className="bg-slate-50 rounded p-3 space-y-1">
                        {extractedData.giftSchedule.map((item: any, idx: number) => (
                          <div key={idx} className="flex justify-between text-sm">
                            <span className="text-slate-600">Year {item.year}:</span>
                            <span className="font-medium">${Number(item.amount?.replace("$", "").replace(",", "")).toLocaleString()}</span>
                          </div>
                        ))}
                      </div>
                    </div>
                  ...
                  {extractedData?.initialSeedGift && (
                    <GiftScheduleDisplay 
                      initialSeedGift={extractedData.initialSeedGift}
                      annualGiftAmount={extractedData.annualGiftAmount}
                      perBeneficiaryAmount={extractedData.perBeneficiaryAmount}
                      grantorNetWorth={extractedData.grantorNetWorth}
                      giftSchedule={extractedData.giftSchedule}
                    />
                  )}
